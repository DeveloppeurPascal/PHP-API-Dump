...
Programme PHP simple pour vérifier ce que vous envoyez d'un script JS (ou d'un formulaire) à un serveur et comment il le reçoit si c'est le cas.

Placez le programme PHP sur un serveur (local ou distant) et vérifiez-le en appelant son URL. S'il est correct, il affichera un objet JSON avec les propriétés POST et GET sous forme de tableaux vides.

(ex : http://localhost/PHP-Dump/src/ajaxcalldump.php comme endpoint ou url d'un appel AJAX)

Utilisez-le pour vérifier votre code JS et voir si ce que vous appelez avec Ajax est bien formaté pour une utilisation PHP sur le serveur.

## Comment faire un dump de votre appel Ajax (ou les formulaires HTML envoyés à un serveur) ?

Le programme PHP se trouve dans le dossier ./src et s'appelle « ajaxcalldump.php ».

Indiquez ce que vous voulez journaliser en mettant à true ou false les paramètres définis en haut du programme.

Indiquez si vous voulez un fichier de dump et où vous le voulez.

Enfin, choisissez si vous voulez que le programme envoie quelque chose en réponse : un objet JSON avec chaque enregistrement, une page HTML ou rien.

### Configuration par défaut

Par défaut le programme exportera en HTML ce qu'il reçoit et créera un fichier dump.txt dans son dossier.

### Sécurité

N'utilisez ce programme que sur un serveur de développement. Ne l'utilisez jamais en production ou supprimez le après vos tests !
